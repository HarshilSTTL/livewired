...
- **Reason:** `creator_profiles.profile_name` uniqueness is now enforced only among
  ...
  `schema/tables/05_creator_profiles.md`). v1's uniqueness check still matched
  against *all* profiles including `status = 'deleted'` ones, so recreating a
  profile with a name that had been deleted always failed with `"Profile name
  already taken"` even though the row itself was gone from the user's point of
  view.
- **Endpoint:** `POST /rpc/create_profile_v2`

### v1 (Deprecated — 2026-09-14)
- Uniqueness check matched against all `creator_profiles` rows regardless of
  `status`, including soft-deleted ones — blocked reusing a deleted profile's name.
- **Endpoint:** `POST /rpc/create_profile` (kept for backwards compatibility)
...
```sql
-- Function: create_profile_v2
-- Group: Profile
-- Endpoint: POST /rpc/create_profile_v2
-- Doc: docs/api/profiles/create_profile.md
-- Version: 2.0 (2026-09-14)
-- Changes: profile_name uniqueness check now excludes soft-deleted profiles
--          (status = 'deleted'), so a deleted profile's name can be reused.
--          Pairs with the partial unique index unique_profile_name_active on
--          creator_profiles (schema/tables/05_creator_profiles.md).

CREATE OR REPLACE FUNCTION create_profile_v2(
    p_user_id             uuid,
    p_profile_name        text,
    ...
    p_youtube_by_default  boolean  DEFAULT false,
    p_rumble_by_default   boolean  DEFAULT false,
...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE profile_name = p_profile_name AND status != 'deleted'
    ) THEN
    ...
    INSERT INTO creator_profiles (
        ...
        youtube_by_default, rumble_by_default,
    ...
    VALUES (
        ...
        p_youtube_by_default, p_rumble_by_default,
        now(), now()
    )
    ...
    RETURN json_build_object(
        ...
        'data', json_build_object(
            ...
            'youtube_by_default', p_youtube_by_default,
            'rumble_by_default',  p_rumble_by_default
        )
    );
...
```sql
-- Function: create_profile (V1 - Deprecated)
-- profile_name uniqueness check includes soft-deleted profiles — a deleted
-- profile's name can never be reused. Use create_profile_v2 for new implementations.
