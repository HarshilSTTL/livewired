# `get_single_profile_by_username`

```sql
-- Function: get_single_profile_by_username
-- Group:    profiles
...
-- Doc:      docs/api/profiles/get_single_profile_by_username.md
--
-- Purpose:  Returns a single profile by its unique username.
--           Used for public profile view pages. Returns any status so the
--           UI can show appropriate messages for suspended/deleted profiles.

CREATE OR REPLACE FUNCTION get_single_profile_by_username(
    p_username text
)
RETURNS JSON
LANGUAGE plpgsql
...
BEGIN
    ...
    -- Check profile exists and get id + show_followers flag
    SELECT id, show_followers
    INTO v_profile_id, v_show_followers
