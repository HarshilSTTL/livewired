...
### v5.0 (Current — 2026-09-14) ✅
- **Function name:** `create_event_v5`
- **Change from v4:** Raises the accepted-collaborator cap from 5 to 9
  (`max_collaborators_per_event` config default and code fallback both updated).
  No other behavior changes.
- **Endpoint:** `POST /rpc/create_event_v5`

### v4.0 (Previous — 2026-08-20)
- **Function name:** `create_event_v4`
- **Change from v3:** Adds `p_event_end_date` so an event can safely run past midnight
  without relying on "end_time < start_time" as an implicit signal.
  - `p_event_end_date` (optional) — defaults to `p_event_date` when not supplied.
  - If provided, must not be before `p_event_date`.
  - End time validation is now a real datetime comparison: the combined
    `(event_end_date, event_end_time)` must be strictly after `(event_date, event_time)`.
    Previously only `event_end_time = event_time` was rejected and `event_end_time <
    event_time` was silently treated as "next day" — that implicit convention is gone;
    the end date is now explicit.
  - Recurring events: the end date shifts per occurrence by the same day offset as the
    start date, so a multi-day recurring event keeps its duration on every occurrence.
- **Endpoint:** `POST /rpc/create_event_v4`

### v3.0 (Previous — 2026-06-30)
...
- **Change from v1:** Default end time behavior
  ...
  - Cross-midnight wrapping handled automatically (e.g. 23:00 → 01:00)
- **Endpoint:** `POST /rpc/create_event_v2`

### v1.0 (Deprecated)
- **Function name:** `create_event`
- No default end time — `event_end_time` stored as NULL when not provided
- **Endpoint:** `POST /rpc/create_event`

---

## V5.0 Function (Current) ✅

```sql
-- Function: create_event_v5
-- Group:    events
-- Endpoint: POST /rpc/create_event_v5
-- Tables:   event_mst (INSERT), event_platforms (INSERT), event_recurring (INSERT if recurring)
-- Doc:      docs/api/events/create_event.md
-- Version:  5.0 (2026-09-14)
-- Change:   Raises the accepted-collaborator cap from 5 to 9 (max_collaborators_per_event).
--           No other behavior changes from v4.
--
-- (v4 change, still applicable) Adds p_event_end_date (optional, defaults to p_event_date). End time
--           validation now compares the full (date, time) datetime pair instead of
--           just the time-of-day, so end_time < start_time is only valid when an
--           explicit later event_end_date makes the combined end datetime later.
--
-- Notes:
...
--   When p_is_recurring = true, create_event_v4 inserts:
...
CREATE OR REPLACE FUNCTION create_event_v5(
    p_profile_id             uuid,
    p_user_id                uuid,
    p_title                  text,
    p_event_date             date,
...
BEGIN
    ...
    -- ── End date defaulting + validation ──────────────────────────────────────
    -- p_event_end_date defaults to the start date when the caller doesn't supply one
    -- (single-day event). If supplied, it cannot be before the start date.
    ...
    -- ── End time validation ───────────────────────────────────────────────────
    -- Compares the full (date, time) datetime pair — end_time < start_time is only
    -- valid when v_event_end_date is later than p_event_date (explicit multi-day span).
    IF (v_event_end_date || ' ' || p_event_end_time)::timestamp <= (p_event_date || ' ' || p_event_time)::timestamp THEN
        RETURN json_build_object('status', false, 'message', 'Event end date/time must be after the event start date/time');
    END IF;

    -- Offset (in days) between start date and end date — reapplied per occurrence
    -- below so a multi-day recurring event keeps its duration on every occurrence.
    v_end_date_offset := v_event_end_date - p_event_date;
    ...
    VALUES (
        ...
        p_event_date, p_event_time, v_event_end_date, p_event_end_time, p_timezone,
            ...
            FOREACH v_day_name IN ARRAY p_recurring_days LOOP
                ...
                WHILE v_occ_date <= v_safe_end LOOP
                    ...
                    VALUES (
                        ...
                        p_title, p_description,
                        v_occ_date, p_event_time, v_occ_date + v_end_date_offset, p_event_end_time, p_timezone,
                        COALESCE(p_livestream, false), COALESCE(p_video, false),
...
## V4.0 Function (Previous)

```sql
-- Function: create_event_v4
-- Group:    events
-- Endpoint: POST /rpc/create_event_v4
-- Tables:   event_mst (INSERT), event_platforms (INSERT), event_recurring (INSERT if recurring)
-- Doc:      docs/api/events/create_event.md
-- Version:  4.0 (2026-08-20)
-- Change:   Adds p_event_end_date (optional, defaults to p_event_date). End time
--           validation now compares the full (date, time) datetime pair instead of
--           just the time-of-day, so end_time < start_time is only valid when an
--           explicit later event_end_date makes the combined end datetime later.
--
...
CREATE OR REPLACE FUNCTION create_event_v4(
    p_profile_id             uuid,
    p_user_id                uuid,
    p_title                  text,
    p_event_date             date,
...
--
-- Superseded by create_event_v4 (2026-08-20) — this version has no event_end_date
-- column and infers cross-midnight events only from end_time < start_time. Kept for
-- existing callers.
--
-- Notes:
--   • event_platforms.platform_id is int4 — cast (pl->>'platform_id')::int4 on INSERT.
...
-- Tables:   event_mst (INSERT), event_platforms (INSERT), event_recurring (INSERT if recurring)
-- Doc:      docs/api/events/create_event.md
-- Version:  1.0 (Deprecated — use create_event_v2)
--
-- Notes:
--   • event_platforms.platform_id is int4 — cast (pl->>'platform_id')::int4 on INSERT.
--   • Validate platform IDs against platforms.plat_id (int8) using ::bigint cast.
