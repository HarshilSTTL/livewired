...
| is_collaborative | bool        | false             | No       | —                              | True = collaborative event (owner + up to 9 accepted collaborators)    |
...
- `is_collaborative = true` enables the collaborator invite system. Up to 9 accepted collaborators allowed per event. Collaborators have **read-only** visibility — they can view the event but cannot update, delete, or postpone it.
